...
describe('tourSteps', () => {
  it('VISITOR_STEPS has 6 entries', () => {
    expect(VISITOR_STEPS).toHaveLength(6)
  })
  ...
  it('visitor steps cover the expected data-tour targets', () => {
    ...
    // #342: the Roadmap tab was dropped from the bottom nav, so its tour step is gone too.
    expect(elements).not.toContain('[data-tour="roadmap"]')
